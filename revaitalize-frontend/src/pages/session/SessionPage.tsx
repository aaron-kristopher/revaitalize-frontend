...
// --- Constants (EXACTLY from original PoseDetection.tsx) ---
const UPPER_BODY_CONNECTIONS = [{ start: 7, end: 2, name: 'ear_to_eyes' }, { start: 2, end: 0, name: 'eyes_to_nose' }, { start: 0, end: 5, name: 'nose_to_eyes' }, { start: 5, end: 8, name: 'eyes_to_ear' }, { start: 9, end: 10, name: 'mouth' }, { start: 11, end: 12, name: 'shoulders' }, { start: 11, end: 13, name: 'left_upper_arm' }, { start: 13, end: 15, name: 'left_lower_arm' }, { start: 12, end: 14, name: 'right_upper_arm' }, { start: 14, end: 16, name: 'right_lower_arm' }, { start: 11, end: 23, name: 'left_torso' }, { start: 12, end: 24, name: 'right_torso' }, { start: 23, end: 24, name: 'hips' }];
...
function SessionPage() {
  ...
  // --- Core Logic State & Refs (EXACTLY from original PoseDetection.tsx) ---
  const webcamRef = useRef<Webcam>(null);
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const lastTimestampRef = useRef<number>(-1);
  const poseLandmarkerRef = useRef<PoseLandmarker | null>(null);
  const animationFrameIdRef = useRef<number | null>(null);
  const [fps, setFps] = useState<number>(0); // Preserved from original
  const fpsTracker = useRef({ frameCount: 0, lastTimeStamp: performance.now() }); // Preserved
  const [isProcessing, setIsProcessing] = useState<boolean>(false); // The original state to control the loop
  const oheExerciseRef = useRef<number[]>([0.0, 1.0, 0.0]);

  // --- Prediction Hook (UNCHANGED) ---
  const { latestPrediction, processFrame, status } = usePoseSequence();

  // --- createPoseLandmarker (UNCHANGED LOGIC) ---
  const createPoseLandmarker = useCallback(async () => {
    try {
      console.log("Creating PoseLandmarker...");
      const vision = await FilesetResolver.forVisionTasks("/wasm");
      const landmarker = await PoseLandmarker.createFromOptions(vision, { baseOptions: { modelAssetPath: "/models/pose_landmarker_full.task", delegate: "GPU" }, runningMode: "VIDEO", numPoses: 1, });
      poseLandmarkerRef.current = landmarker;
      console.log("PoseLandmarker Created Successfully.");
    } catch (error) { console.error("Failed to create PoseLandmarker!", error); }
  }, []);
  ...
  const detect = useCallback(() => {
    ...
    if (!ctx) { console.error("Failed to Load 2d context from canvas!"); return }
    ...
    if (results && results.landmarks && results.landmarks.length > 0) {
      ...
      if (latestPrediction) { latestPrediction.forEach((value, index) => { if (value === 1) { const keypointIndex = PREDICTION_TO_KEYPOINT_MAP[index]; if (keypointIndex !== undefined) { errorIndices.add(keypointIndex); } } }); }
      for (const connection of UPPER_BODY_CONNECTIONS) { const startLandmark = allLandmarks[connection.start]; const endLandmark = allLandmarks[connection.end]; if (startLandmark && endLandmark) { const isError = errorIndices.has(connection.start) || errorIndices.has(connection.end); const color = isError ? '#C70039 ' : '#50C878'; ctx.beginPath(); ctx.moveTo(startLandmark.x * canvasWidth, startLandmark.y * canvasHeight); ctx.lineTo(endLandmark.x * canvasWidth, endLandmark.y * canvasHeight); ctx.lineWidth = 4; ctx.strokeStyle = color; ctx.stroke(); } }
      for (const index of UPPER_BODY_INDICES) { const landmark = allLandmarks[index]; if (landmark) { const isError = errorIndices.has(index); const color = isError ? '#C70039 ' : '#FFFFFF'; ctx.beginPath(); ctx.arc(landmark.x * canvasWidth, landmark.y * canvasHeight, 7, 0, 2 * Math.PI); ctx.fillStyle = color; ctx.fill(); } }
    }
  }, [processFrame, latestPrediction]);

  // --- Animation Loop useEffect (UNCHANGED, driven by 'isProcessing') ---
  useEffect(() => {
    const runAnimation = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (poseLandmarkerRef.current) { console.log("Closing PoseLandmarker from Detection"); poseLandmarkerRef.current.close(); poseLandmarkerRef.current = null; }
    };
  }, [createPoseLandmarker]);

  // --- UI HANDLERS (The Bridge Between UI and Original Logic) ---
  const handleTogglePlay = () => {
    if (sessionState === 'idle') { setSessionState('running'); setIsProcessing(true); } 
    else if (sessionState === 'paused') { setSessionState('running'); setIsProcessing(true); } 
    else if (sessionState === 'running') { setSessionState('paused'); setIsProcessing(false); }
  };
  const handleEnd = () => {
    setSessionState('idle'); setIsProcessing(false);
    setFeedback({ status: 'waiting', text: 'Press Play to Begin' }); setCurrentReps(0); setAccuracy(0); setCurrentSet(1);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-100 text-slate-900">
      ...
      <main className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-4 p-4 lg:p-6 bg-slate-800 overflow-y-auto">
        <div className="w-full min-h-[300px] lg:min-h-[400px] bg-black rounded-xl lg:rounded-2xl flex items-center justify-center relative overflow-hidden">
          {/* LOGIC FIX: Webcam is only rendered when not 'idle', turning the camera on/off */}
          {sessionState !== 'idle' ? (
            <>
              <Webcam ref={webcamRef} className="w-full h-full object-cover" mirrored={false} videoConstraints={{ facingMode: 'user' }} />
