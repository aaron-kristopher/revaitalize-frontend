...
function SessionPage() {
  ...
  const { latestPrediction, latestPredictionRef, processFrame } = usePoseSequence();
  ...
  const [mostFrequentError, setMostFrequentError] = useState<string>("No Error");
  ...
  const currentRepPredictions = useRef<number[][]>([]); // Will hold the binary values
  const currentRepEvaluations = useRef<string[]>([]); // Will hold the string representation (i.e. Correct, leftArm, etc.)
  const sessionScores = useRef<number[]>([]);
  const sessionErrorFlag = useRef<string[]>([]);
  ...
  const getPredictionEvaluation = (prediction: number[]) => {

    const leftShoulderIndex = 0;
    const rightShoulderIndex = 1;
    const leftElbowIndex = 2;
    const rightElbowIndex = 3;
    const leftWristIndex = 4;
    const rightWristIndex = 5;

    let evaluation: string = "Correct";

    if (prediction[leftShoulderIndex]) {

      if (prediction[rightShoulderIndex])
        evaluation = "Incorrect Posture: Both Shoulders";

      evaluation = "Incorrect Posture: Left Shoulders";

    } else if (prediction[rightShoulderIndex])
      evaluation = "Incorrect Posture: Right Shoulders";

    if (prediction[leftElbowIndex]) {

      if (prediction[rightElbowIndex])
        evaluation = "Incorrect Posture: Both Elbows";

      evaluation = "Incorrect Posture: Left Elbows";

    } else if (prediction[rightElbowIndex])
      evaluation = "Incorrect Posture: Right Elbows";

    if (prediction[leftWristIndex]) {

      if (prediction[rightWristIndex])
        evaluation = "Incorrect Posture: Both Wrists";

      evaluation = "Incorrect Posture: Left Wrists";

    } else if (prediction[rightWristIndex])
      evaluation = "Incorrect Posture: Right Wrists";
  ...
  const runLiveDetection = useCallback(() => {
    ...
    if (results.landmarks?.length > 0) {
      ...
      if (currentPrediction) {
        currentRepEvaluations.current.push(getPredictionEvaluation(currentPrediction));
        currentRepPredictions.current.push(currentPrediction);
      }
  ...
  }, [poseLandmarker, exerciseName, processFrame])
  ...
  }, [runLiveDetection, isProcessing, latestPrediction, mostFrequentError]);
  ...
  const getMostFrequentError = (currentRepEvaluations: string[]) => {
    const errorCount: Record<string, number> = {};
    const excludedEvaluation: string = "Correct"
    let maxErrors: number = 0;
    let frequentError: string = "No Error"

    for (const evaluation of currentRepEvaluations) {
      if (evaluation !== excludedEvaluation) {
        errorCount[evaluation] = (errorCount[evaluation] || 0) + 1;
      }
    }

    for (const error in errorCount) {
      if (errorCount[error] > maxErrors) {
        maxErrors = errorCount[error];
        frequentError = error;
      }
    }
  ...
  const handleRepComplete = useCallback(async () => {
    ...
    const totalPredictions: number = currentRepPredictions.current.length;
    const qualityScore: number = totalPredictions > 0
      ? ((totalPredictions - totalErrors) / totalPredictions) * 100
      : 100; // Default to 100% if no predictions, avoiding division by zero
    setMostFrequentError(getMostFrequentError(currentRepEvaluations.current))

    sessionScores.current.push(qualityScore);
    sessionErrorFlag.current.push(mostFrequentError);

    // Reset for next rep
    currentRepPredictions.current = [];
    currentRepEvaluations.current = [];
  ...
  useEffect(() => {
    ...
    if (latestPrediction) {
      ...
      setFeedback({
        ...
        text: evaluation === "No Error" ? "Excellent From!" : evaluation
      });
    }
